...
@foreach ($content as $row)
    ...
    <li class="c-search-destinations__item">
        <a href="{{route('destination.show', [$row])}}" class="c-search-destinations__item-image" style="background-image: url({{\App\Image::getRandom()}});"></a>
        <div class="c-search-destinations__item-content">
            <h3 class="c-search-destinations__item-title">
                <a href="{{route('destination.show', [$row])}}" class="c-search-destinations__item-title-link">{{$row['name']}}</a>
            </h3>
