      ...
      ->with(
        ...
        component('Meta')->with(
          ...
          collect()
            ->pushWhen(
              $commentCount > 0,
              component('Tag')->with('title', trans_choice('comment.count', $commentCount, ['count' => $commentCount]))
            )
            ->push(component('MetaLink')->with('title', $blog->user->vars()->name))
            ...
            ->pushWhen(
              $loggedUser && $loggedUser->hasRoleOrOwner('admin', $blog->user->id),
              component('MetaLink')
            ...
            ->pushWhen(
              ...
              component('Form')
                ...
                ->with(
                  'fields',
                  collect()->push(
                    component('FormLink')->with('title', trans("content.action.status.$blog->status.title"))
                  )
                )
            )
        )
