        ...
        collect()
          ...
          ->push(
            component('Meta')->with(
              'items',
              collect()
                ->push(component('MetaLink')->with('title', $travelmate->vars()->created_at))
                ->pushWhen(
                ...
                ->pushWhen(
                  ...
                  component('Form')
                    ->with('route', route('content.status', [$travelmate->type, $travelmate, 1 - $travelmate->status]))
                    ->with(
                      'fields',
                      collect()->push(
                        component('FormLink')->with('title', trans("content.action.status.$travelmate->status.title"))
                      )
                    )
                )
            )
          ...
          ->push(
            component('Body')
              ->is('responsive')
              ->with('body', $travelmate->vars()->body)
          )
          ->push(region('Share'))
          ->merge(
