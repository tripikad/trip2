                        ...
                        collect(array_fill(0, 5, null))
                          ->map(function ($value, $key) use ($isPackage) {
                            return collect()
                              ...
                              ->push(
                                component($isPackage ? 'FormTextfield' : 'FormHidden')
                                  ->with('title', trans('offer.admin.edit.hotel.rating') . ' ' . ($key + 1))
                                  ->with('name', 'hotel_rating[]')
                                //->with('options', '')
                              )
                              ->push(
                                component($isPackage ? 'FormTextfield' : 'FormHidden')
                                  ->with('title', trans('offer.admin.edit.hotel.type') . ' ' . ($key + 1))
                                  ->with('name', 'hotel_type[]')
                                //->with('options','')
                              )
                              ->push(
                                component($isPackage ? 'FormTextfield' : 'FormHidden')
                                  ->with('title', trans('offer.admin.edit.hotel.price') . ' ' . ($key + 1))
