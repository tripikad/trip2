...
class StatisticsController extends Controller
{
  public function index()
  {
    return layout('Two')
      ...
      ->with(
        'header',
        region('StaticHeader', collect()->push(component('Title')->with('title', trans('statistics.index.title'))))
      )
        ...
        collect()->merge(
          collect(['User', 'Content', 'Comment', 'Flag'])->flatMap(function ($model) {
            return collect()
              ...
              ->push(component('Linechart')->with('items', $this->getMonthlyStat($model)))
              ->push(
                component('Title')
                  ->is('small')
                  ->with('title', trans('statistics.index.weekly'))
              )
              ->push(component('Linechart')->with('items', $this->getWeeklyStat($model)));
          })
        )
      )
