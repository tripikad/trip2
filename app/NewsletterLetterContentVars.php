...
class NewsletterLetterContentVars
{
    ...
    protected $allowed_order_by_columns = ['id', 'pop', 'updated_at', 'created_at'];

    protected $allowed_order_by_options = ['asc', 'desc'];

    protected $allowed_types = ['flight', 'forum', 'buysell', 'expat', 'travelmate', 'news', 'shortnews'];
    ...
    public function compose(Content $the_flight = null, NewsletterType $newsletterType = null)
    {
        ...
        return preg_replace('/(\>)\s*(\<)/m', '$1$2', preg_replace(['/\s{2,}/', '/[\t\n]/'], ' ', $this->temp_body));
    }
    ...
    protected function replace_if_matches(Content $the_flight = null, NewsletterType $newsletterType = null)
    {
            ...
            foreach (['type', 'take', 'skip', 'order_by'] as $type) {
                if (strpos($this->tags, $type . ':') !== false) {
                    ...
                    if ($type == 'type') {
                        ...
                        ($type == 'take' || $type == 'skip') &&
                        strpos($column_values, ',') === false &&
                        is_numeric($column_values) &&
                        (int) $column_values > 0
                    ) {
        ...
        if ($skip_element) {
            ...
            if ($contents) {
                ...
                $contents = Content::with(['images', 'user', 'user.images', 'destinations', 'topics'])->where(
                    'status',
                    1
                );
                $check_contents = Content::with(['images', 'user', 'user.images', 'destinations', 'topics'])->where(
                    'status',
                    1
                );
                ...
                if (
                    isset($sql_options['order_by']) &&
                    isset($sql_options['order_by']['column']) &&
                    isset($sql_options['order_by']['option'])
                ) {
                    if ($sql_options['order_by']['column'] == 'pop') {
                        ...
                        $check_contents = $check_contents
                            ->select([
                                'contents.*',
                                DB::raw(
                                    '(SELECT COUNT(*) FROM comments WHERE content_id = contents.id) AS comments_count'
                                )
                            ])
                        ...
                        $contents = $contents->orderBy(
                            $sql_options['order_by']['column'],
                            $sql_options['order_by']['option']
                        );
                    }
                }
                ...
                foreach ($contents as &$item) {
                    if ($item->type == 'flight') {
                        ...
                        $content .= mail_component('mail::list', [
                            'image' => filter_var($user_image, FILTER_VALIDATE_URL)
                                ? $user_image
                                : ($user_image
                                    ? asset($user_image)
                                    : null),
